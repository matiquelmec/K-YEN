...
export async function POST(request: NextRequest) {
  try {
    ...
    const collectionId = (formData.get('collectionId') as string) || 'collection';
    ...
    const safeFileName = `collection-${collectionId}-${Date.now().toString().slice(-6)}`;
    const uploadResult = await uploadToCloudinary(buffer, file.type, 'collections', safeFileName);
