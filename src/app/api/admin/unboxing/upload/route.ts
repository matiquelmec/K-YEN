...
export async function POST(request: NextRequest) {
  try {
    ...
    const safeFileName = `unboxing-packaging-${Date.now().toString().slice(-6)}`;
    ...
    console.error('Error uploading unboxing image:', error);
    return NextResponse.json(
      { error: error.message || 'Error al subir la imagen de packaging' },
      { status: 500 }
    );
  }
}
